...
FinestraClienteSelezionato::FinestraClienteSelezionato(DatabaseClienti* d, DatabaseComponenti* db, Cliente* c, QDialog *parent):QDialog(parent), dbc(d), dbComp(db), cl(c){
    ...
    // new tabella
    tabellaSale=new QTableWidget(0,8);
    tabellaSale->setColumnWidth(0,150);
    QStringList header;
    header=QStringList() << "Nome" << "Stabilimento" << "Tipo Impianto" << "Kw Tot." << "Portata Effettiva (Lt/s)" << "Pressione Richiesta (Bar)" << "Portata Richiesta (Lt/s)" << "Valore sala (€)";
    ...
    tabellaSale->setColumnWidth(0,200);
    tabellaSale->setColumnWidth(1,180);
    tabellaSale->setColumnWidth(2,150);
    tabellaSale->setColumnWidth(3,80);
    tabellaSale->setColumnWidth(4,150);
    tabellaSale->setColumnWidth(5,150);
    tabellaSale->setColumnWidth(6,150);
    tabellaSale->setColumnWidth(7,150);
...
void FinestraClienteSelezionato::riempiTabellaSale() {
    ...
    for (unsigned int i= 0; i<cl->getSala().size();++i) {
        ...
        QTableWidgetItem* itemValoreSala= new QTableWidgetItem (QString::number(cl->getSala(i).getValoreSala()));
        tabellaSale->setItem(row,7, itemValoreSala);
        itemValoreSala->setFlags(Qt::ItemIsSelectable|Qt::ItemIsEnabled);
        ++row;
    }
}
